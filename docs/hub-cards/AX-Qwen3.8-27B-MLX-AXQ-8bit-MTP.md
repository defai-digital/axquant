...
# AX-Qwen3.8-27B-MLX-AXQ-8bit-MTP — 8.63 BPW measured main
...
> `7772fd6eb7b9` — measured size against a matched uniform baseline, quality retention,
...
> See the [checkpoint Tier 1 certificate](https://github.com/defai-digital/axquant/blob/main/docs/certifications/qwen38-27b-axq8-mtp-tier1.md) for the bound evidence and thresholds.
...
| AXQuant base precision class | `8p8bpw` |
| Planned storage-adjusted BPW | 8.7397 |
| Measured main-model BPW | 8.6271 |
| Measured total BPW, including MTP | **8.7398** |
| Safetensors weight size | 30.35 GB |
| Approximate complete download | 30.37 GB |
...
hf download AutomatosX/AX-Qwen3.8-27B-MLX-AXQ-8bit-MTP --local-dir ./AX-Qwen3.8-27B-MLX-AXQ-8bit-MTP
```

Allow at least 30.37 GB of free disk space. Pin the resulting Hub commit in reproducible
...
mlx_lm.generate \
  --model AutomatosX/AX-Qwen3.8-27B-MLX-AXQ-8bit-MTP \
...
```bash
ax-engine serve ./AX-Qwen3.8-27B-MLX-AXQ-8bit-MTP --port 31418
```
...
```bash
mtplx quickstart \
  --model ./AX-Qwen3.8-27B-MLX-AXQ-8bit-MTP \
  --profile stable \
  --depth 1 \
  --reasoning off
```
...
| MTP acceptance and speed | **not certified** for public Tier 2. A short Studio probe on `df-macstudio-m2` / AX Engine 6.16.1 cleared exactness and ~1.23× speedup (`full=false`, 8+6 prompts). That is not the authorizing `--full` suite. See the [Tier 2 evaluation](https://github.com/defai-digital/axquant/blob/main/docs/certifications/qwen38-27b-axq8-mtp-tier2.md). |
...
| Release certification | **Checkpoint Tier 1 certified** on `df-macstudio-m2` (2026-08-15), Hub commit `7772fd6eb7b9`; the formal AXQuant M0-M8 release campaign is a separate process and is not implied |
