...
# AX-Ornith-1.5-9B-MLX-AXQ-6bit-MTP — 8.20 BPW measured main
...
| AXQuant base precision class | `9p0bpw` |
| Planned storage-adjusted BPW | 8.4010 |
| Measured main-model BPW | 8.2046 |
| Measured total BPW, including MTP | **8.4011** |
| Safetensors weight size | 10.14 GB |
| Approximate complete download | 10.16 GB |
...
hf download AutomatosX/AX-Ornith-1.5-9B-MLX-AXQ-6bit-MTP --local-dir ./AX-Ornith-1.5-9B-MLX-AXQ-6bit-MTP
```

Allow at least 10.16 GB of free disk space. Pin the resulting Hub commit in reproducible
...
mlx_lm.generate \
  --model AutomatosX/AX-Ornith-1.5-9B-MLX-AXQ-6bit-MTP \
...
```bash
mtplx quickstart \
  --model ./AX-Ornith-1.5-9B-MLX-AXQ-6bit-MTP \
  --profile stable \
  --depth 1 \
  --reasoning off
```
