...
2. Every new algorithm output is a **new candidate label** in sensitivity and
   ...
   forever.
3. Evidence schemas are versioned additively. A report writer may add fields;
   a change to the meaning of an existing field requires a new schema version
   string and a migration note in `docs/`.
4. Phase 0 (certification closure) takes priority over all other phases. No
