...
| 3 | M2 | Harness, layout backend, phase profiler, and the named exact-profile measurement contract complete; the gate passes on uniform-6 but not yet on any size-compliant candidate | The 2026-08-01 formal-protocol M5 measurement puts the AXQ-026 candidate at 1.0969x (direct 32.68 tok/s, MTP 35.85, exactness pass, acceptance 0.8955); depth-2 drafts are exact for the first time (AX Engine 08df0fdd) but lose to ~12 ms/verify-token scaling. The single quantified lever is removing the ~5.1 ms/cycle CPU verify-graph build (wiring the existing per-layer compile infrastructure into the 2-token verify), whose modeled effect is ~1.24x |
