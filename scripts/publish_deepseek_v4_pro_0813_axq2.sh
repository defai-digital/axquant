...
echo "confirm Hub lists weights before Ext12T cleanup"
if ! "$HF" repo files "$REPO" --repo-type model | grep -E 'safetensors|axquant_manifest.json'; then
  echo "Hub did not list weights; keeping Ext12T copies (SC-R6)" >&2
  exit 2
fi
...
echo "cleanup Ext12T after Hub confirm (PRD SC-R6)"
