#!/bin/bash
# Factory-only: plan + convert Flash-0731 2-bit attn6 v0.2, then T1 v-extract.
...
WORK=/Volumes/Ext16TR0/axquant/work/flash-0731-2bit-attn6
OUT=/Volumes/Ext16TR0/models/AX-DeepSeek-V4-Flash-0731-MLX-AXQ-2bit-attn6
RECIPE=$ROOT/examples/deepseek-v4-experimental-2bit-attn6-v0.2.yaml
LOG=$WORK/logs
...
echo "===== $(date -u +%Y-%m-%dT%H:%M:%SZ) plan 2bit-attn6 ====="
"$AXQ" plan-manual \
  --inventory "$INV" \
  --recipe "$RECIPE" \
  --output "$WORK/plans/2bit-attn6.json" \
  --markdown-output "$WORK/plans/2bit-attn6.md"
python3 - <<PY
import json
from collections import Counter
p = json.load(open("$WORK/plans/2bit-attn6.json"))
...
echo "===== $(date -u +%Y-%m-%dT%H:%M:%SZ) convert 2bit-attn6 ====="
if [ -f "$OUT/config.json" ] && ls "$OUT"/*.safetensors >/dev/null 2>&1; then
  ...
  "$AXQ" convert \
    --model "$SRC" \
    --revision "$REV" \
    --plan "$WORK/plans/2bit-attn6.json" \
    --allow-unmeasured \
...
export AX_ENGINE_VERSION=80f2a3e6-attn6
