#!/bin/bash
# Factory-only: plan + convert Flash-0731 2-bit shared4 v0.4, then T1 v-extract.
...
WORK=/Volumes/Ext16TR0/axquant/work/flash-0731-2bit-shared4
OUT=/Volumes/Ext16TR0/models/AX-DeepSeek-V4-Flash-0731-MLX-AXQ-2bit-shared4
RECIPE=$ROOT/examples/deepseek-v4-experimental-2bit-shared4-v0.4.yaml
...
echo "===== $(date -u +%Y-%m-%dT%H:%M:%SZ) plan 2bit-shared4 ====="
"$AXQ" plan-manual \
  --inventory "$INV" \
  --recipe "$RECIPE" \
  --output "$WORK/plans/2bit-shared4.json" \
  --markdown-output "$WORK/plans/2bit-shared4.md"
python3 - <<PY
import json
from collections import Counter
p = json.load(open("$WORK/plans/2bit-shared4.json"))
...
echo "===== $(date -u +%Y-%m-%dT%H:%M:%SZ) convert 2bit-shared4 ====="
if [ -f "$OUT/config.json" ] && ls "$OUT"/*.safetensors >/dev/null 2>&1; then
  ...
  "$AXQ" convert \
    --model "$SRC" \
    --revision "$REV" \
    --plan "$WORK/plans/2bit-shared4.json" \
    --allow-unmeasured \
...
export AX_ENGINE_VERSION=80f2a3e6-shared4
