#!/usr/bin/env python3
"""Factory: convert + checkpoint Tier 1 for Qwen3.8-27B AXQ-MXFP4-MTP.

Run on df-macstudio-m2 with Ext16TR0:

  PYTHONPATH=src /Users/devop/code/axquant-main/.venv/bin/python \\
    scripts/run_qwen38_27b_axq_mxfp4_mtp.py all

Converts the full BF16 source (integrated MTP kept at BF16). Uniform size
reference is the existing no-MTP MXFP4 convert with a slightly looser gate.

Environment: QWEN38_BF16, QWEN38_MXFP4_MTP_PACK, QWEN38_MXFP4_MTP_WORK,
...
WORK = Path(
    os.environ.get(
        "QWEN38_MXFP4_MTP_WORK",
        "/Volumes/Ext16TR0/axquant-certification/qwen38-27b-axq-mxfp4-mtp-tier1",
    )
)
BF16 = Path(os.environ.get("QWEN38_BF16", "/Volumes/Ext16TR0/models/Qwen3.8-27B-bf16"))
PACK = Path(
    os.environ.get(
        "QWEN38_MXFP4_MTP_PACK",
        "/Volumes/Ext16TR0/models/AX-Qwen3.8-27B-MLX-AXQ-MXFP4-MTP",
    )
)
DATASETS = Path(
    os.environ.get(
...
HUB_NAME = "AX-Qwen3.8-27B-MLX-AXQ-MXFP4-MTP"
CERT_STEM = "qwen38-27b-axq-mxfp4-mtp-tier1"
RECIPE = ROOT / "examples" / "qwen38-27b-axq-mxfp4-mtp-v0.1.yaml"
MAX_SIZE_RATIO = 1.28
MIN_QUALITY = 0.98
SEED = 20260728
MAX_TOKENS = 64
...
def cmd_uniforms() -> None:
    ...
    source = source_bf16()
    reused = Path(
        "/Volumes/Ext16TR0/axquant-certification/qwen38-27b-axq-mxfp4-tier1/uniforms/uniform-mxfp4"
    )
    ...
    if (reused / "config.json").is_file() and any(reused.glob("*.safetensors")):
        ...
        log(f"copied no-MTP uniform MXFP4 reference {reused} -> {out}")
        return
    out.parent.mkdir(parents=True, exist_ok=True)
...
def cmd_write_certs() -> None:
    ...
    payload = {
        ...
        "public_index": {
            "display_name": "Qwen3.8-27B MLX AXQ MXFP4 MTP",
            "sort_order": 6,
